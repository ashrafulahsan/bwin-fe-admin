...
const GRID_COLUMNS = "minmax(0,2.2fr) minmax(0,1fr) minmax(0,0.8fr) minmax(240px,1fr)";
const headerCellStyle = { fontSize: "var(--fs-caption)", fontWeight: "var(--fw-medium)", color: "var(--text-muted)", letterSpacing: "0.02em" };

export default function ArticleTable({ rows, noRows, emptyMessage }) {
  return (
    <>
      <div style={{ display: "grid", gridTemplateColumns: GRID_COLUMNS, gap: 14, alignItems: "center", padding: "10px 18px", background: "var(--surface-sunken)", borderBottom: "1px solid var(--border)" }}>
        <span style={headerCellStyle}>Post</span>
        <span style={headerCellStyle}>Category &amp; tags</span>
        ...
        <div key={p.id} style={{ display: "grid", gridTemplateColumns: GRID_COLUMNS, gap: 14, alignItems: "start", padding: "14px 18px", borderBottom: "1px solid var(--border)", opacity: p.opacity }}>
          <div style={{ minWidth: 0, display: "flex", alignItems: "flex-start", gap: 12 }}>
            ...
            <div style={{ minWidth: 0, flex: 1, display: "flex", flexDirection: "column", gap: 5 }}>
              ...
              <span style={{ fontSize: "var(--fs-body-sm)", color: "var(--text-secondary)", lineHeight: 1.5, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>{p.excerpt}</span>
              <span style={{ fontSize: "var(--fs-caption)", color: "var(--text-muted)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                <span style={{ fontFamily: "var(--font-mono)" }}>{p.slugDisplay}</span>
                {p.metaTail}
              </span>
            </div>
          </div>
          ...
          <div style={{ minWidth: 0, display: "flex", flexDirection: "column", gap: 6 }}>
            <span style={{ fontSize: "var(--fs-body-sm)", fontWeight: "var(--fw-medium)", color: "var(--text-primary)" }}>{p.categoryName}</span>
            ...
            {p.noTags && <span style={{ fontSize: "var(--fs-caption)", color: "var(--text-muted)" }}>No tags</span>}
          </div>
